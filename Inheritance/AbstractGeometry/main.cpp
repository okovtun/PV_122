...
namespace Geometry
{
	...
	public:
		...
		void draw()const
		{
			...
			HDC hdc = GetDC(hwnd);
			//3) Создаем карандаш:
			HPEN hPen = CreatePen(PS_SOLID, 5, color);//PS_SOLID - сплошная линия, 5 - толщина линии 5 пикселов
			//4) Прежде чем рисорвать , нужно выбрать чем и на чем рисовать:
			SelectObject(hdc, hPen);

			//5) Чтобы фигура была закрашена определенным цветом, нужно создать и применить кисть
